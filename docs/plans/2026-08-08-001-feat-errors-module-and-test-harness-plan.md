...
- KTD6. **`build.type = "none"`.** The documented null build back-end. `build_rockspec()` initializes `rockspec.build.type` before processing dependencies, defaulting an absent type to `builtin`; `--only-deps` then returns after dependency resolution and skips the build driver, the only place the type is acted on. Declaring `none` is what stops that `builtin` default from standing, and makes a stray plain `luarocks make` an intentional no-op rather than module auto-discovery.
